/**
 * Refresh analytics_citizen_features mart from OLTP tables.
 * Usage: npx tsx scripts/refresh-features.ts
 */
...
async function main() {
  ...
  const [result] = await conn.query(REFRESH_SQL);
  console.log("Feature mart refreshed:", result);
  await conn.end();
}
